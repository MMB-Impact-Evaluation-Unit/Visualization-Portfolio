...
# filter to year 2016 - 2019 so the map loads faster
borehole <- water %>%
...
pal <- colorFactor(palette = c("#003685", "#A6192E", "#FFC845", "#5D295F", "#78BE21"),
...
# create title object
title <- tags$div(tag.map.title, HTML("Borehole Water Points"))
...
borehole %>%
  leaflet() %>%
  addTiles(tilesURL) %>%
  addCircles(lng = ~lon_deg, lat = ~lat_deg, color = ~pal(water_tech), opacity = 0.8,
             weight = 7) %>%
  addLegend("topright", pal = pal, values = ~water_tech, title = "Water Tech Type",
            opacity = 1) %>%
