...
# filter data: county (USA/China/India/Indonesia) ,  year(1995, 2000, 2005, 2010)
data <- population %>% 
  filter(country %in% c("United States of America", "China", "India", "Indonesia")) %>% 
...
data %>% 
  ...
  geom_bar(aes(fill = country), stat = "identity", show.legend = FALSE) + #turn off legend
 ...
 # scale_y_continuous(breaks = seq(0, 350, by = 50)) + # adjust y axis limits, grid line by 50
  labs(
    title = "Population of Four Countries",
  ...
  theme(plot.title = element_text(hjust = 0.5), #center title
        ...
        strip.text = element_text(size = 12, face = "bold"), #"facet title" size/bold 
